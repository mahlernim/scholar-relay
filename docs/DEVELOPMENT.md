...
### v1.3.2 publication and store submission

The user authorized publication on their behalf. [GitHub v1.3.2](https://github.com/mahlernim/scholar-relay/releases/tag/v1.3.2) was published at `2026-09-04T11:22:41Z` from the verified draft, targeting `2a7ef7a3265d8c99da83ef525059ecf407955d03`. The latest documentation-only merged-main CI also passed in [run 33867394049](https://github.com/mahlernim/scholar-relay/actions/runs/33867394049). The canonical ZIP was not rebuilt. Its 26 entries, embedded version, decompressed bytes, 114,101-byte size, and SHA-256 `5921bf2adea872eb4ea23378d3399f925f1775852a287581bfc461a6716963a5` were reverified before upload.

The same ZIP was accepted by existing Chrome Web Store item `epopghhfmpokhbalmnfcopmplffphdbb`. The Package page showed Draft 1.3.2, all seven languages, and unchanged permissions. Published remained 1.3.0. All seven descriptions were saved and read back against the prepared copy. English uses two updated global screenshots. Korean, Japanese, Spanish, French, German, and Brazilian Portuguese each have two matching localized screenshots. Existing reviewer instructions remain applicable.

Submission succeeded on September 4, 2026, with the confirmation “Your extension was submitted for review” and dashboard status “Pending review”, verified by `2026-09-04T11:29:59Z`. Automatic publication after approval was checked. Pending review is not public availability. Verify the public version and localized listing after approval. Keep #14 open until post-publication search evidence is recorded, and keep unrelated #25 open. Published v1.3.1 remains unchanged.

### September 5 review fixes

Issues #32 through #35 cover notebook ID parsing, uncertain generation outcomes, initial media readiness, and absent source statuses. Notebook creation now decodes the Project ID field at index 2 rather than searching title strings. The reference examined was `teng-lin/notebooklm-py` commit `6494d409fa6fae09b32fd4ce696fc28c1d7c52fd`, particularly `_web/rows/notebooks.py` and `_web/rows/sources.py`. Creation and polling apply the same media readiness requirement, and source polling requires explicit readiness. Malformed mutation envelopes remain uncertain, including calls that permit an explicit null acknowledgement. An unidentified generation request retains inspection guidance and its diagnostic instead of claiming that Google generated nothing. Arbitrary newly listed artifacts are not adopted without reliable attribution.

Local validation passed 93 tests and the Chrome smoke, including the real 40 MiB transfer, seven locales, and expandable uncertain-artifact diagnostics. Existing published packages were not rebuilt. Issue #36 tracks the subsequent persistent queue and clear handoff states. Issue #25 remains separate because the upstream reference still describes `pending_review` semantics as unconfirmed.

### v1.4.0 paper queue preparation

The September 5 review fixes merged in [PR #37](https://github.com/mahlernim/scholar-relay/pull/37), closing #32 through #35. Both checks passed at head `9052d22a008e1670c6e203755ee7ed671df960dc`. [Merged-main CI](https://github.com/mahlernim/scholar-relay/actions/runs/33938501040) passed at `c067890d9c29a4f525fc3b5353123e75e60830a2`.

Issue #36 replaces the single pipeline record with a serialized, persistent job queue. The queue prepares one paper at a time and monitors up to three generating notebooks. Each job keeps source identity, settings, notebook and artifact IDs, diagnostics, and ownership checks. Pausing affects new starts. Stopping a job preserves other jobs and remote work already accepted by Gemini Notebook. Permission waits release the preparation slot. Resuming a PDF fallback joins the queue instead of taking another paper's slot. Interrupted writes remain uncertain and are not replayed.

Local PDFs commit to IndexedDB before enqueue acknowledgment. Per-file and combined bounds are 40 MiB and 100 MiB, with 20 unfinished jobs and 50 finished history entries. Files are removed after upload, failure, or removal. Startup cleans orphaned files and migrates the legacy pipeline record. Queue records and alarm changes share one writer so stale cleanup cannot remove a newer job's wakeup. Authentication and reads have bounded waits, as do PDF downloads and uploads. Accepted mutation response bodies retain their existing non-abort policy.
...
### v1.4.2 remaining issue review

Issues #42, #46, #47, and #48 describe reproducible defects. Version 1.4.2 reads notebook titles only from the confirmed Project title slot, preserves multi-line JSON response records, shares concurrent authentication token fetches, and scales the final upload deadline from 25 seconds to 185 seconds at the 40 MiB limit. It also avoids rebuilding unchanged progress markup, adds keyboard and ARIA behavior to collapsible artifact settings, distinguishes stopped progress from active work, and retains completed artifact details. Short acronyms and two-character East Asian titles are accepted while generic filenames remain excluded. PDF Info literals now preserve balanced parentheses, and hex strings decode raw backslash bytes without treating them as escapes.
...
### v1.5.0 completion preferences and cancelled starts
